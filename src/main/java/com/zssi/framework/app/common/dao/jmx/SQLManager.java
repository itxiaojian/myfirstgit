...
package com.zssi.framework.app.common.dao.jmx;
...
import com.zssi.framework.app.common.dao.support.CustomSQL;
import com.zssi.framework.app.common.dao.support.CustomSQL.SQLBean;
...
public class SQLManager implements SQLManagerMBean {
	...
	public String findSQL(String id) {
		SQLBean bean = CustomSQL.getInstance().getAllSQL().get(id);
		if(bean == null)
			return "sql id不存在";
		else
	...
	public SQLManager() {
		try {
	        ...
	        for(Entry<String, SQLBean> entry : CustomSQL.getInstance().getAllSQL().entrySet()) {
