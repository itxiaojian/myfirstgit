...
import com.zssi.framework.app.process.dao.ProcessApiXNDao;
...
public class ProcessXNService {
	...
	public Pagination<Map<String, Object>> getRunningProcessList(Integer start, Integer limit, String ypbh, String ypmc) {
		Pagination<Map<String, Object>> pagination = processApiDao.getRunningProcess(start, limit, ypbh, ypmc);
				...
				try{
				    ...
				    for (ActivityImpl activityImpl : ((ProcessDefinitionEntity) processDefinition).getActivities()) {
				    	if(activityId.equals(activityImpl.getId())) {
				    		mapResult.put("name", activityImpl.getProperty("name"));
					    	break;
				    	}
				    }						
				} catch(Exception e) {
	...
	public Pagination<Map<String, Object>> getInvolvedProcess(Integer start, Integer limit, String ypbh, String ypmc,String rolekind) {
		...
		Pagination<Map<String, Object>> pagination = processApiDao.getInvolvedProcess(start, limit, userCode, ypbh, ypmc,rolekind);
		//根据本次分页结果查询出对应的业务数据
		for(Map<String, Object> mapResult: pagination.getResult()) {
	...
	public Pagination<Map<String, Object>> getHistoryProcessList(Integer start, Integer limit, String ypbh, String yplx) {
		Pagination<Map<String, Object>> pagination = processApiDao.getHistoryProcess(start, limit, ypbh, yplx);
		return pagination;
	}
	...
	private List<String> getHighlightedFlows(List<PvmTransition> pvmTransitionList, LinkedList<HistoricActivityInstance> hisActInstList, boolean isParallel){
		 ...
		 */
		@Transactional
		public void forceOverProcess(String processInstanceId, String deleteReason, String ypbh) {
			runtimeService.deleteProcessInstance(processInstanceId, deleteReason);
			businessDao.forceProcess(ypbh);
		}	
		...
		public Pagination<Map<String, Object>> getRwch(Integer start, Integer limit, String ypbh, String ypmc,String rolekind) {
			SysYh user = AppUtil.getCurrentUser();
			String userCode = user.getUsername();
			Pagination<Map<String, Object>> pagination = processApiDao.getRwch(start, limit, userCode, ypbh, ypmc,rolekind);
			//根据本次分页结果查询出对应的业务数据
			for(Map<String, Object> mapResult: pagination.getResult()) {
				String processInstanceId = mapResult.get("processInstanceId").toString();
				String processDefinitionId = mapResult.get("processDefinitionId").toString();
		...
		public Pagination<Map<String, Object>> getBhypxg(Integer start, Integer limit, String ypbh, String ypmc,String rolekind) {
			SysYh user = AppUtil.getCurrentUser();
			String userCode = user.getUsername();
			Pagination<Map<String, Object>> pagination = processApiDao.getBhypxg(start, limit, userCode, ypbh, ypmc,rolekind);
			//根据本次分页结果查询出对应的业务数据
			for(Map<String, Object> mapResult: pagination.getResult()) {
				String processInstanceId = mapResult.get("processInstanceId").toString();
				String processDefinitionId = mapResult.get("processDefinitionId").toString();
