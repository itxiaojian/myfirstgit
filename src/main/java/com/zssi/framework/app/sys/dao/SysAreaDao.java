...
//					map.put("checked",false);
	 ...
	 * @return
	 */
	public List<Map<String, Object>> findBz(String bmbh){
		String str = "";
	...
	public List<Map<String,Object>> getOrgTree(String node){
		...
		String sql = "select org.BMBH as \"id\",org.BMMC as \"text\" from sys_zzjg org where org.JB <> '4' and ";
		SysYh user = AppUtil.getCurrentUser();    //获得session用户
	...
	public List<Map<String,Object>> getOrgTree1(String node){
		String userCode = null;
		String sql = "select org.ID as \"id\",org.BMMC as \"text\" from sys_zzjg org where org.JB <> '9' and ";
		SysYh user = AppUtil.getCurrentUser();    //获得session用户
		userCode = user.getUsername();
		if(userCode.equals("admin")){
		   // sql = "select org.id as \"id\",org.name as \"text\" from UNTECK_ORGANIZATION org where ";
	...
	public List<Map<String, Object>> getchildtree(String parentid) {
		String sql="SELECT A.ID AS \"id\",A.BMMC AS \"text\",A.SJBH AS \"sjbh\",(SELECT COUNT(1) FROM SYS_ZZJG B WHERE B.SJBH=A.ID ) AS \"cnt\" FROM SYS_ZZJG A WHERE SJBH = "+parentid+"  ORDER BY px";
		return jdbcTemplate.queryForList(sql);
	}
	
	public List<Map<String, Object>> gettext(String id) {
		String sql="SELECT BMMC AS \"text\" FROM SYS_ZZJG WHERE col_id = "+id;
		return jdbcTemplate.queryForList(sql);
	}
}
