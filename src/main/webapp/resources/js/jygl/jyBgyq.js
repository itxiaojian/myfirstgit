var USER_GRID_STORE_URL = 'getBgyq';
var PAGESIZE=20;
            ...
            cm: new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
            	...
            	{header:'操作',dataIndex:'',width:130,sortable: true,
            		renderer:function(value, cellmeta, record){
					   return "<a href='javascript:;' onclick='constructionGrid.onBgyq()' style='text-decoration:none;'>" +
					   		  "<span style='width: 26px;cursor: pointer;'>报告延期</span></a>"+
					   		  "&nbsp&nbsp&nbsp"+
					   		  "<a href='javascript:;' onclick='constructionGrid.onYqck()' style='text-decoration:none;'>"+
					   		  "<span style='width: 26px;cursor: pointer;'>延期查看</span></a>";
					}
            	},
...
//            	{header:'认证方式',dataIndex:'RZFS',width:100,sortable: true},
    ...
    onBgyq: function() {                  //报告延期
    	var records=this.getSelectionModel().getSelections();
   		if(records.length > 0) {
   			if(records.length == 1){
   				vrecord = records[0];
   				var win = this.bgyqWindow;
   		    	win.show();
   		    	var winForm = win.constructionForm;
   		    	win.constructionForm.bgmc.setValue(vrecord.get('BGMC'));
    			win.constructionForm.bgbh.setValue(vrecord.get('BGBH'));
    			win.constructionForm.bzrxm.setValue(vrecord.get('BZR'));
    			win.constructionForm.jsdw.setValue(vrecord.get('JSDW'));
    			win.constructionForm.wcqx.setValue(vrecord.get('WCQX'));
   			}else{
    ...
    onYqck: function() {                  //延期查看
    	var records=this.getSelectionModel().getSelections();
   		if(records.length > 0) {
   			if(records.length == 1){
   				vrecord = records[0];
   				var win = this.yqckWindow;
   		    	win.show();
   		    	var winForm = win.constructionForm;
   		    	winForm.store.load({params:{start:0,limit:PAGESIZE,bgbh:vrecord.get('BGBH')}});
   			}else{
   				Ext.Msg.alert('系统提示', '不能查看多条记录..');
   			}
   		}else{
