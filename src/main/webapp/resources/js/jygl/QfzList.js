var USER_GRID_STORE_URL = 'getBgdy';
var PAGESIZE=20;
...
ConstructionGrid = Ext.extend(UxGrid, {
    ...
    constructor: function(height, width){
    	this.store = new Ext.data.Store({          //Grid Store
            ...
            reader: new Ext.data.JsonReader({totalProperty: 'total', root:'rows'},[
                            {name:'ID'},{name:'BGBH'},{name:'YPMC'},{name:'SHSJ'},{name:'BZR'},{name:'DYCS'},
...
//            	{header:'打印次数',dataIndex:'DYCS',width:100,sortable: true},
            	{header:'审核时间',dataIndex:'SHSJ',width:100,sortable: true},
            	...
            	{header:'检验结论',dataIndex:'JYJL',width:300,sortable: true},
    	...
    	var records=this.getSelectionModel().getSelections();
   		if(records.length > 0) {
   			if(records.length == 1){
    			...
    			var bgbh=vrecord.get('BGBH');
   				Ext.Ajax.request({
		       		url: PROJECT_NAME + '/jygl/YjyBgxx/cxywpdf', 
			       	   method : 'POST', 
			       	   params: { bgbh: bgbh},
		               success: function(form, action) {
		            	   var respText = Ext.util.JSON.decode(form.responseText);
		            	   if(respText == '2'){
		            		   window.open(PROJECT_NAME + '/jygl/YjyBgxx/Qfz?bgbh='+vrecord.get('BGBH'));
		            	   }else if(respText == '1'){
		            		   Ext.MessageBox.alert("系统提示:", BLANKSTR + "该报告未生成PDF格式!" + BLANKSTR);
		            	   }
		               },
		               failure: function(form, action) {
