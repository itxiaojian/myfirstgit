var USER_GRID_STORE_URL = 'getJcInfoList';
...
//    	this.idHidden = this.createHidden('id','id');
    	...
    	this.khyf =  new Ext.form.DateField({
			fieldLabel: '考核月份',
			name: "khyf",
			format: "Y年-m月",
			anchor: '90%',
    	...
    	this.jcqk = new Ext.form.TextArea({
            fieldLabel: '奖惩情况',
            name: 'jcqk',
            readOnly: false,
            anchor: '90%',
    	...
    	this.khyf.allowBlank = false;
    	this.jcqk.allowBlank = false;
    ...
    constructor: function(height, width){
    	this.store = new Ext.data.Store({          //Grid Store
            ...
            reader: new Ext.data.JsonReader({totalProperty: 'total', root:'rows'},[
                            {name:'ID'},{name:'RYBH'},{name:'RYXM'},{name:'KHYF'},
                            {name:'JCQK'},{name:'BZ'}
            ])
        });
...
//                '-',{text:'修改',iconCls: 'edit',handler:this.onModifyClick,scope:this},
                ...
                '-',{xtype:'textfield',id:'code',width: 150,
    	  			...
    	  			'-',{xtype:'button',text:'查询',iconCls:'query',handler:function(){
   		    		...
   		    		Ext.Msg.confirm('系统提示','确定要下载该模板吗?',function(btn){
		        	    if(btn=="yes"){ 
		        	    	window.open(PROJECT_NAME+"/resources/js/rsgl/rsjcinfo.xls");
		        	   }
		        	 });
		    	 },scope:this},
              ...
              '-',{xtype:'button',text:'下载查询结果',iconCls:'excel',handler:function(){
   		    		Ext.Msg.confirm('系统提示','确定下载查询结果吗?',function(btn){
		        	    if(btn=="yes"){ 		        	    	
		        	    	var fileName = "奖惩管理信息";
		        	    	var code = Ext.getCmp('code').getValue();
		        	    	var url = PROJECT_NAME + "/rsgl/YRsJcInfo/export?fileName="+fileName+"&code="+code;
		        	    	url = encodeURI(url);
		        	    	url = encodeURI(url);
		        	    	window.open(url);
		        	   }
        ...
        ConstructionGrid.superclass.constructor.call(this, {
            ...
            cm: new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
                ...
                {header:'人员编号',dataIndex:'RYBH',width:120,sortable: true, hidden:true},
                {header:'人员姓名',dataIndex:'RYXM',width:120,sortable: true},
            	{header:'考核月份',dataIndex:'KHYF',width:120,sortable: true},
            	{header:'奖惩情况',dataIndex:'JCQK',width:120,sortable: true},
...
//    	win.constructionForm.getForm().reset();
    	var url = "jcAddView?id="+id;  	
		html = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>';    		
		ACT_DEAL_WINDOW = new ActDealWindow();
		ACT_DEAL_WINDOW.setTitle("奖惩信息增加");
    ...
    onModifyClick: function() {
		...
		var url = "jcXgView?id="+id;  	
		html = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>';    		
		ACT_DEAL_WINDOW = new ActDealWindow();
		ACT_DEAL_WINDOW.setTitle("奖惩信息修改");
		ACT_DEAL_WINDOW.html = html;
		ACT_DEAL_WINDOW.show();
    },
