var INVOLVED_PROCESS_GRID_STORE_URL = 'rwchProcess/list';
var BLANKSTR = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'; 
var PAGESIZE=20;
var ACT_DEAL_WINDOW;
...
InvolvedProcessGrid = Ext.extend(UxGrid, {
    ...
    constructor: function(height, width){
    	this.store = new Ext.data.Store({
            ...
            reader: new Ext.data.JsonReader({totalProperty: 'total', root:'rows'},[
            		...
            		{name:'createTime'},{name:'dueDate'},{name:'description'},{name:'ypbh'},{name:'ypmc'},{name:'ypid'}
    	...
    	this.vtbar = new Ext.Toolbar({
            items:[
            	...
            	'-',{xtype:'button',text:'清空',iconCls:'redo',handler:function(){
  	   						...
  	   						Ext.getCmp('ymmc_query').setValue("");
     			  		}
            		},
            ]
        });
            ...
            cm: new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
              ...
              {header:'操作',dataIndex:'asd',width:100,sortable: true,
            	  renderer:function(value, cellmeta, record){
            		  return "<a class='redlink' title='任务撤回' href='javascript:void(0)' onclick='involvedProcessGrid.rwch();'>任务撤回</a>";
            	  }
              },
    ...
    rwch: function() {
    	...
    	var valueStr=[];
   		if(records.length>0) {
	    	...
	    	Ext.Msg.confirm("提醒信息", "确定要撤回这 " + records.length + " 条信息吗",function(btn){
				if (btn == 'yes') {
			       	Ext.Ajax.request({
			       		url: PROJECT_NAME + '/cps/deal/dealWebCh', 
				       	   method : 'POST', 
				       	   params: { id: valueStr},
			               success: function(form, action) {
				               Ext.MessageBox.alert("系统提示:", BLANKSTR + "撤回成功!" + BLANKSTR);
				               involvedProcessGrid.store.reload({params:{start:0,limit:PAGESIZE}});
			               },
			               failure: function(form, action) {
			            	   Ext.MessageBox.alert("系统提示:", BLANKSTR + "撤回失败!" + BLANKSTR);
			               }
				       	});					
				}
	    	});	
    ...
    onQueryClick: function() {
		...
		var ypmc = Ext.getCmp("ymmc_query").getValue();
