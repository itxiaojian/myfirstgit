    	...
    	if(YPCS=="0"){
    		 tss=new Ext.Toolbar({
    			items:[
    		       ...
    		       '-',{text:'批量签收',iconCls: 'add',handler:this.onQsClick,scope:this},
    		       '-',{text:'批量办理',iconCls: 'add',handler:this.onBlClick,scope:this}
    		       ]
    		});
//    	}else if(YPCS=="2"){
//    		tss=new Ext.Toolbar({
...
//    		       '-',{text:'批量退回',iconCls: 'add',handler:this.onThClick,scope:this},
//    		       ]
//    		});
    	}else{
    		 tss=new Ext.Toolbar({
...
//	                   	}
                       if(value == null || value == "" ) {
                           return '<a class="zlink" href="javascript:void(0)" onclick="pendPoolGrid.signFor();">签收</a>&nbsp&nbsp&nbsp&nbsp';
                       }else {
                           return '<a class="redlink" href="javascript:void(0)" onclick="pendPoolGrid.dealTask();">办理</a>&nbsp&nbsp&nbsp&nbsp';
                       }
                    }
                },  
...
/*            	{header:'流程实例ID',dataIndex:'processInstanceId',width:100,sortable: true},
    ...
    onQsClick:function(){     //批量签收
    	var records=this.getSelectionModel().getSelections();
    	var valueStr=[];
    	var k=0;
   		if(records.length>0) {
	       	for(var j=0;j<records.length;j++){
	       		var assignee = records[j].get('assignee');
	       		if(assignee !=  "" && assignee != null){        //状态为已签收（办理）
	       			k++;
	       		}
    	 	}
	       	if(k==0){
	       		...
	       		Ext.Msg.confirm("提醒信息", "确定要批量签收这 " + records.length + " 条信息吗",function(btn){
       				if (btn == 'yes') {
       					Ext.Ajax.request({
       						url: 'PLclaimTask', 
       						method : 'POST', 
       						params: { taskIds: valueStr},
       						success: function(form, action) {
       							Ext.MessageBox.alert("系统提示:", BLANKSTR + "批量签收成功!" + BLANKSTR);
       							pendPoolGrid.store.reload({params:{start:0,limit:PAGESIZE}});
       						},
       						failure: function(form, action) {
       							Ext.MessageBox.alert("系统提示:", BLANKSTR + "批量签收失败!" + BLANKSTR);
       						}
       					});					
       				}
       			});
	       	}else{
	       		Ext.Msg.alert('系统提示', BLANKSTR + "请重新选择需要签收的任务" + BLANKSTR);
	       	}
   		}else{
	   		 Ext.Msg.alert('系统提示', BLANKSTR + '请至少选择一条记录' + BLANKSTR);
	      	return;
   		}
    },
    
    onBlClick:function(){         //批量办理
    	var records=this.getSelectionModel().getSelections();
    	var taskIds=[];
    	var businessKeys=[];
    	var k=0;
   		if(records.length>0) {
   			...
   			if(k==0){
   				for(var i=0;i<records.length;i++){
   					...
   					businessKeys.push(records[i].get('businessKey'));
   				}
   				Ext.Msg.confirm("提醒信息", "确定要批量分配这 " + records.length + " 条信息吗",function(btn){
   					if (btn == 'yes') {
   						var url = "PLopenTaskDealPage?taskIds="+taskIds+"&businessKeys="+businessKeys+"&fl=1";   	
   						html = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>';    		
   						ACT_DEAL_WINDOW = new DealWindow();
   						ACT_DEAL_WINDOW.setTitle("任务批量处理");
   						ACT_DEAL_WINDOW.html = html;
   						ACT_DEAL_WINDOW.show();
   					}
   				});
   			}else{
	       		Ext.Msg.alert('系统提示', BLANKSTR + "请重新选择需要办理的任务" + BLANKSTR);
	       	}
		}else{
   		 	Ext.Msg.alert('系统提示', BLANKSTR + '请至少选择一条记录' + BLANKSTR);
   		 	return;
    ...
    onThClick:function(){       //批量退回
    	...
    	if(records.length>0) {
	    	...
	    	Ext.Msg.confirm("提醒信息", "确定要批量退回这 " + records.length + " 条信息吗",function(btn){
					if (btn == 'yes') {
						var url = "PLopenTaskDealPage?taskIds="+taskIds+"&businessKeys="+businessKeys+"&fl=2";   	
						html = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>';    		
						ACT_DEAL_WINDOW = new DealWindow();
						ACT_DEAL_WINDOW.setTitle("任务批量退回");
						ACT_DEAL_WINDOW.html = html;
						ACT_DEAL_WINDOW.show();
					}
				});
    ...
    onRefreshClick: function() {
    	var ypcs = pendPoolGrid.store.getAt(0).get("ypcs");
    	pendPoolGrid.store.load({params:{start:0,limit:PAGESIZE}});  
    },
...
/***************************************DealWindow组件**************************************************/
DealWindow = Ext.extend(Ext.Window,{
    constructor: function(grid) {
    	DealWindow.superclass.constructor.call(this, {
            width: 500,
            anchor: '50%',
            maximized :false,
            height: 300,
            ...
            autoScroll: true,
            closeAction: 'close',
            html:'<iframe scrolling="auto" frameborder="0" width="40" height="50" src=""></iframe>'
        });
    }
});
