var USER_GRID_STORE_URL = 'getBgsfList';
var PAGESIZE=20;
...
//            	'-',{text:'删除',iconCls: 'delete',handler:this.onDeleteClick,scope:this},
        ...
        ConstructionGrid.superclass.constructor.call(this, {
            ...
            cm: new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),
                ...
                {header: '操作', width: 120, align:"center",sortable: true,hidden: false,
					renderer:function(value, cellmeta, record){
						if(record.get("YSFJE") != 0){
							   return "<a href='javascript:;' onclick='constructionGrid.onLookjl()' style='text-decoration:none;'>" +
								...
								return"<a href='javascript:;' onclick='constructionGrid.onLookjl()' style='text-decoration:none;'>" +
								   	  ...
								   	  "<span style='width: 26px;color:red;'>删除</span></a>";
								}
						}
			    },
			    {header:'收费状态',dataIndex:'SFZT',width:60,sortable: true,
            		renderer:function(value, cellmeta, record){
            			if(record.get("YSFJE") !=0){
            				if(record.get("JYFY") > record.get("YSFJE")) {
            					return "<span style=color:blue>收费中</span>";
            					}else if(record.get("JYFY") == record.get("YSFJE")) {
            						return "<span style=color:blue>已收费</span>";
            						}else{
            							return "<span style=color:gray>信息有误</span>";
            							}
            				}else if(record.get("YSFJE") ==0){
            					return "<span style=color:red>待收费</span>";
            				}
            			}
           	    },
                {header:'报告编号',dataIndex:'BGBH',width:100,sortable: true},
                {header:'样品名称',dataIndex:'YPMC',width:100,sortable: true},
                {header:'受检单位',dataIndex:'SJDW',width:100,sortable: true},
                {header:'委托单位',dataIndex:'WTDW',width:100,sortable: true},
            	{header:'检验科室',dataIndex:'KS_ID',width:100,sortable: true},
            	{header:'业务科室',dataIndex:'SSYWKS',width:100,sortable: true},
            	{header:'登记时间',dataIndex:'JYJSRQ',width:100,sortable: true, hidden:true},
           	 ...
           	 {header:'收费人',dataIndex:'SFR',width:100,sortable: true,hidden:true},
    	...
    	var records=this.getSelectionModel().getSelections();
   		if(records.length > 0) {
   			if(records.length == 1){
   				...
   				var bgbh=records[0].get('BGBH');
//   				alert(id);
				var url = "bgsfonSfeiView?id="+id+"&bgbh="+bgbh;
				html = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>';    		
				ACT_DEAL_WINDOW = new ActDealWindow();
				ACT_DEAL_WINDOW.setTitle("检验缴费");
				ACT_DEAL_WINDOW.html = html;
				ACT_DEAL_WINDOW.show();
   			}else{
	   				Ext.Msg.alert('系统提示', '不能修改多条记录..');
