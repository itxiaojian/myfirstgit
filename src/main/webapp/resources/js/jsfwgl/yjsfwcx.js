...
//			var cxtj=$('#cxtj').val();
		...
		function showQj(value){
			...
			if(value=='between'){
				...
				str=str+"</select>";
			}else if(xzzd=='JYKS_ID'){
					str="<label>关键字：</label>" +
					"<select class='form-control1 input-lg1 m-bot15 textinput' name='gjz' id='gjz' style='width:40%;margin-top: 10px;'>" +
					"<option value=''>请选择...</option>";
				for(var i=0;i<jyksidList.length;i++){
					str=str+"<option value='"+jyksidList[i]+"'>"+jybmmcList[i]+"</option>";
				}
				str=str+"</select>";
			}else if(xzzd=='FKFS'){
					str="<label>关键字：</label>" +
					"<select class='form-control1 input-lg1 m-bot15 textinput' name='gjz' id='gjz' style='width:40%;margin-top: 10px;'>" +
					"<option value=''>请选择...</option>";
				for(var i=0;i<fkfszdzList.length;i++){
					str=str+"<option value='"+fkfszdzList[i]+"'>"+fkfszdmcList[i]+"</option>";
				}
				str=str+"</select>";
		    }else if(xzzd=='DJRQ'||xzzd=='ZZRQ'||xzzd=='SXRQ'){
		    	$('#xztj').val('=');
				...
				var str="";
				var xzzd=$('#xzzd').val();
				if(xzzd=='JYKS_ID'||xzzd=='YWKS_ID'||xzzd=='KS_ID'||xzzd=='FKFS'){
					alert("该属性请选择等于条件！");
			    	return false;
				}else if(xzzd=='DJRQ'||xzzd=='ZZRQ'||xzzd=='SXRQ'){
					str="<label>关键字：</label><input name='gjz' id='gjz' type='text' style='width:75%;margin-left: 5px;' class='form-control1 Wdate textinput' onclick=\"WdatePicker({dateFmt:'yyyy-MM-dd'})\" truetype='textinput' readonly='readonly'/>";
		...
		function addTj(){
			...
			if(xztj=='between'){
				...
				if(gjz==null||gjz==''){
					...
					if(xzzd=='KS_ID'||xzzd=='JYKS_ID'||xzzd=='YWKS_ID'||xzzd=='FKFS'){
		    ...
		    var str = "<table cellpadding='0' cellspacing='0' border='0' class='display table table-bordered'><thead><tr>";
		   	...
		   	for(var i = 0; i < data.length; i++){
		   		...
		   		for(var j=0;j<values.length ;j++){
		   			...
		   			if(values[j]=='XYBH'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].XYBH)+"</td>";
		   			}
		   			if(values[j]=='KHMC'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].KHMC)+"</td>";
		   			}
		   			if(values[j]=='KHDZ'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].KHDZ)+"</td>";
		   			}
		   			if(values[j]=='FRDB'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].FRDB)+"</td>";
		   			}
		   			if(values[j]=='LXDH'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].LXDH)+"</td>";
		   			}
		   			if(values[j]=='CPMC'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].CPMC)+"</td>";
		   			}
		   			if(values[j]=='FWXM'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].FWXM)+"</td>";
		   			}
		   			if(values[j]=='XYLX'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].XYLX)+"</td>";
		   			}
		   			if(values[j]=='SXRQ'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].SXRQ)+"</td>";
		   			}
		   			if(values[j]=='ZZRQ'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].ZZRQ)+"</td>";
		   			}
		   			if(values[j]=='KS_ID'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].KS_ID)+"</td>";
		   			}
		   			if(values[j]=='XYZY'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].XYZY)+"</td>";
		   			}
		   			if(values[j]=='XYKH'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].XYKH)+"</td>";
		   			}
		   			if(values[j]=='FKFS'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].FKFS)+"</td>";
		   			}
		   			if(values[j]=='BZ_ID'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].BZ_ID)+"</td>";
		   			}
		   			if(values[j]=='JYBH_ID'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].JYBH_ID)+"</td>";
		   			}
		   			if(values[j]=='KHJLGM'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].KHJLGM)+"</td>";
		   			}
		   			if(values[j]=='KHHZ_INFO'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].KHHZ_INFO)+"</td>";
		   			}
		   			if(values[j]=='XYFZR'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].XYFZR)+"</td>";
		   			}
		   			if(values[j]=='DJRQ'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].DJRQ)+"</td>";
		   			}
		   			if(values[j]=='JYKS_ID'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].JYKS_ID)+"</td>";
		   			}
		   			if(values[j]=='YEKS_ID'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].YEKS_ID)+"</td>";
		   			}
		   			if(values[j]=='BZ'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].BZ)+"</td>";
		   			}
		   			if(values[j]=='SYJE'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].SYJE)+"</td>";
		   			}
		   			if(values[j]=='XMLXR'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].XMLXR)+"</td>";
		   			}
		   			if(values[j]=='DHHM'){
		   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].DHHM)+"</td>";
		   			}
