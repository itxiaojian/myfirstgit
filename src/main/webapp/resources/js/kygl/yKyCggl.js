var USER_GRID_STORE_URL = 'getKyCgglList';
var PAGESIZE=20;
var ENTITY_URL_UPLOAD = 'upload';
var RWZP_URL1 = '/yrwgl/yrwzp/getRwZprList';
var ENTITY_URL_UPLOAD1 = "/kygl/YkyCggl/uploadImage";
...
ConstructionForm = Ext.extend(Ext.ux.Form, {
    ...
    constructor: function() { 
        ...
        this.kybh = this.createTextField('科研编号', 'kybh', '90%','',null,100,'长度超过不能50');
    	...
    	this.kyxmnr = this.createTextField('科研项目内容', 'kyxmnr', '90%','',null,100,'长度超过不能50');
    	
//    	this.ssks = this.createTextField('所属科室', 'ssks', '90%','',null,100,'长度超过不能50');
    	
    	this.ssks = new zjyw.OrgSingelSelectAll('所属科室','ssks','ssks','90%');
    	...
    	this.cglx = this.createTextField('成果类型', 'cglx', '90%','',null,100,'长度超过不能11');
    	
//    	this.cgnr = this.createTextField('成果内容', 'cgnr', '99%','',null,100,'长度超过不能11');
    	
    	this.cgnr = new Ext.ux.form.FileUploadField({
			anchor:'95%',
			name: 'attachMentFile',
            emptyText: '请选择...',
            labelWidth: 67,
    	...
    	this.kybh.allowBlank = true;
        this.kymc.allowBlank = true;
        this.kyxmnr.allowBlank = true;
        this.ssks.allowBlank = true;
        this.cyry.allowBlank = true;
        this.kssj.allowBlank = true;
        this.jssj.allowBlank = true;
        this.cglx.allowBlank = true;
        this.cgnr.allowBlank = true;
     ...
     addFormClick: function(){
    			if(this.getForm().isValid()){
    				var attachMentFileValue = this.cgnr.getValue();
...
//    	        	var attachmentType = attachMentFileValue.substring(attachMentFileValue.lastIndexOf(".")+1).toLowerCase();
//    	        	if(attachmentType != "jpg" or "png" or "jpeg" or "bmp" or "gif" or "png" or "pdf" or "doc" or "docx" or "mpp" or "txt" or "xls" or "xlsx" or "zip" or "rar" or "ppt" or "pptx" ){
//    	        		Ext.MessageBox.alert("系统提示:", BLANKSTR + "请上传正确类型的文件" + BLANKSTR);
//    	        		return;
//    	        	}
...
ConstructionGrid = Ext.extend(UxGrid, {
    ...
    constructor: function(height, width){
    	this.store = new Ext.data.Store({          //Grid Store
            ...
            reader: new Ext.data.JsonReader({totalProperty: 'total', root:'rows'},[
                            {name:'ID'},{name:'KYBH'},{name:'KYMC'},{name:'KYXMNR'},{name:'SSKS'},
                            {name:'CYRY'},{name:'KSSJ'},{name:'JSSJ'},{name:'CGLX'},{name:'CGNR'},{name:'BZ'}
            ])
        });
    	this.vbbar= this.createPagingToolbar(PAGESIZE);
    	this.vtbar = new Ext.Toolbar({
                ...
                '-',{xtype:'textfield',id:'code',width: 150,
                  ...
                  '-',{xtype:'button',text:'下载EXCEL模板',iconCls:'excel',handler:function(){
     		    		Ext.Msg.confirm('系统提示','确定要下载该模板吗?',function(btn){
    		        	    if(btn=="yes"){ 
    		        	    	window.open(PROJECT_NAME+"/resources/js/kygl/Cgglxx.xls");
    		        	   }
    		        	 });
    		    	 },scope:this},
                  ...
                  '-',{xtype:'button',text:'下载查询结果',iconCls:'excel',handler:function(){
  	   		    		Ext.Msg.confirm('系统提示','确定下载查询结果吗?',function(btn){
  			        	    if(btn=="yes"){ 		        	    	
  			        	    	var fileName = "成果管理信息";
  			        	    	var code = Ext.getCmp('code').getValue();
  			        	    	var url = PROJECT_NAME + "/kygl/YkyCggl/export?fileName="+fileName+"&code="+code;
  			        	    	url = encodeURI(url);
  			        	    	url = encodeURI(url);
  			        	    	window.open(url);
  			        	   }
...
//				},
            	...
            	{header:'所属科室',dataIndex:'SSKS',width:150,sortable: true},
            	{header:'参与人员',dataIndex:'CYRY',width:100,sortable: true},
            	{header:'开始时间',dataIndex:'KSSJ',width:90,sortable: true, hidden:true},
            	{header:'结束时间',dataIndex:'JSSJ',width:90,sortable: true, hidden:true},
            	{header:'成果类型',dataIndex:'CGLX',width:80,sortable: true, hidden:true},
            	{header:'成果内容',dataIndex:'CGNR',width:160,sortable: true},
...
//    	win.constructionForm.getForm().reset();
		...
		ACT_DEAL_WINDOW.setTitle("新增成果信息")
    ...
    onModifyClick: function() {                  //修改
    	...
    	var records = this.getSelectionModel().getSelections();
 		...
 		var url  = "cgglXgView?id="+id;  	
 		html = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>';    		
 		ACT_DEAL_WINDOW = new ActDealWindow();
 		ACT_DEAL_WINDOW.setTitle("修改成果信息");
 		ACT_DEAL_WINDOW.html = html;
 		ACT_DEAL_WINDOW.show();
   		if(records.length > 0) {
   			if(records.length == 1){
