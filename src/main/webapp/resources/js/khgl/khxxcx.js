...
//			var cxtj=$('#cxtj').val();
		...
		function showQj(value){
			...
			if(value=='between'){
			    var str="";
			    var xzzd=$('#xzzd').val();
			    if(xzzd=='CLSJ'||xzzd=='LRSJ'||xzzd=='SR'){
				...
				str=str+"</select>";
				}else if(xzzd=='CLSJ'||xzzd=='LRSJ'||xzzd=='SR'){
			    	$('#xztj').val('=');
...
//				var str="<label>关键字：</label><input name='gjz' id='gjz' type='text' style='width:75%;margin-left: 5px;' class='form-control1 textinput'/>";
		...
		function showTj(value){
			...
			if(xztj!=null&&xztj!=''){
				if(xztj=='between'){
					var str="";
					if(value=='CLSJ'||value=='LRSJ'||value=='SR'){
						str="<label>起始：</label><input name='qjq' id='qjq' type='text' style='width:34%;margin-left: 5px;' class='form-control1 Wdate textinput' onclick=\"WdatePicker({dateFmt:'yyyy-MM-dd'})\" truetype='textinput'/>"
							+ "<label style='margin-left: 4%;'>截止：</label><input name='qjz' id='qjz' type='text' style='width:34%;margin-left: 5px;' class='form-control1 Wdate textinput' onclick=\"WdatePicker({dateFmt:'yyyy-MM-dd'})\" truetype='textinput'/>";
						var oTest = document.getElementById("Div2");
						var newNode = document.createElement("div");
	    ...
	    var str = "<table cellpadding='0' cellspacing='0' border='0' class='display table table-bordered'><thead><tr>";
	   	...
	   	for(var i = 0; i < data.length; i++){
	   		...
	   		for(var j=0;j<values.length ;j++){
	   			...
	   			if(values[j]=='KHBH'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].KHBH)+"</td>";
	   			}
	   			if(values[j]=='KHMC'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].KHMC)+"</td>";
	   			}
	   			if(values[j]=='KHFL'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].KHFL)+"</td>";
	   			}
	   			if(values[j]=='DWXZ'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].DWXZ)+"</td>";
	   			}
	   			if(values[j]=='KHDZ'){
	   			...
	   			if(values[j]=='GDDH'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].GDDH)+"</td>";
	   			}
	   			if(values[j]=='CZ'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].CZ)+"</td>";
	   			}
	   			if(values[j]=='FRXM'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].FRXM)+"</td>";
	   			}
	   			if(values[j]=='CLSJ'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].CLSJ)+"</td>";
	   			}
	   			if(values[j]=='ZCZJ'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].ZCZJ)+"</td>";
	   			}
	   			if(values[j]=='JYCPMC'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].JYCPMC)+"</td>";
	   			}
	   			if(values[j]=='LRSJ'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].LRSJ)+"</td>";
	   			}
	   			if(values[j]=='BZ'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].BZ)+"</td>";
	   			}
	   			if(values[j]=='ZJLX'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].ZJLX)+"</td>";
	   			}
	   			if(values[j]=='ZJHM'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].ZJHM)+"</td>";
	   			}
	   			if(values[j]=='SR'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].SR)+"</td>";
	   			}
	   			if(values[j]=='DS'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].DS)+"</td>";
	   			}
	   			if(values[j]=='DWWZ'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].DWWZ)+"</td>";
	   			}
	   			if(values[j]=='YWJE'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].YWJE)+"</td>";
	   			}
	   			if(values[j]=='DZYX'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].DZYX)+"</td>";
	   			}
	   			if(values[j]=='FRQQ'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].FRQQ)+"</td>";
	   			}
	   			if(values[j]=='SJHM1'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].SJHM1)+"</td>";
	   			}
	   			if(values[j]=='GDDH1'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].GDDH1)+"</td>";
	   			}
	   			if(values[j]=='CZ1'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].CZ1)+"</td>";
	   			}
	   			if(values[j]=='DZYX1'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].DZYX1)+"</td>";
	   			}
	   			if(values[j]=='LLQQ'){
	   				str=str+"<td class='center hidden-phone'>"+isNull(data[i].LLQQ)+"</td>";
	   			}
	   		}
	   		str=str+"</tr>";
	   	}
