...
//	alert(imgeId);
	...
	// if(CusName.value == "")
	// {
	strFileName = path + imgeId.toString() + szPostfix;
	// }
	// else 
	// {
...
function SetMergePageType(iValue){
	...
	var path = document.getElementById("path").value;
	var dstFileName = path + imgeId.toString() + szPostfix;//合并的目标文件名
//	alert(dstFileName);
//	alert(imgeId);
	if(strMergeSource1!=null && strMergeSource2!=null)
	{
		...
		{
			alert("将"+strMergeSource1+"和"+strMergeSource2+"合并成功，合并方式为"+iMergePageType.toString());
		}
		else if(Capture.MergeImages(strMergeSource1,strMergeSource2,dstFileName,iMergePageType)==-1)
		{
			alert("操作成功");
		}
	}
