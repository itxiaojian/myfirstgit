		...
		function getValueFys(){
			...
			$.ajax({
	            ...
	            type: "POST",
	            url: 'getJyyjlist',
	            data: {start: start, limit: size, size: count, cplx: cplx, cpbh: cpbh, cpmc: cpmc, jyyj: jyyj},
	            async: false,
...
//		页面刚进来时执行的方法，动态分别加载表格
		window.onload = function(){
			...
			$.ajax({
	            ...
	            type: "POST",
	            url: 'getJyyjlist',
	            data: {start: 0, limit: size,size: count, cplx: cplx, cpbh: cpbh, cpmc: cpmc, jyyj: jyyj},
	            async: false,
	            error: function (request) {
	                alert("链接失败,请联系管理员。");
	            },
		...
		function toView1(cpbh,cplx,jyyj ){
		...
		function creatTable(data){
		    ...
		    var str = "<table cellpadding='0' cellspacing='0' border='0' class='display table table-bordered'>" 
		                ...
		                +"<td class='hidden-phone'><b>科室名称</B></td>"
		                +"<td class='hidden-phone'><b>产品编号</B></td>"
		                +"<td class='hidden-phone'><b>产品名称</B></td>"
		                +"<td class='hidden-phone'><b>产品类型</B></td>"
		                +"<td class='hidden-phone'><b>检验依据</B></td>"
		                +"<td class='hidden-phone'><b>操作</td></B>";
		   	...
		   	for(var i = 0; i < data.length; i++){
		   			str=str+"<td class='center hidden-phone' style='color:red;'>"+(i+1)+"</td>"
			   			   ...
			   			   +"<td class='center hidden-phone'><a href='javascript:;' onclick='toView1(\""+data[i].CPBH+"\",\""+data[i].CPLX+"\",\""+data[i].JYYJ+"\")' class='paginate_disabled_previous'>查询检验项目</a></td>";;
			   		str=str+"</tr>";
			}
		   	str=str+"</tbody></table>";
