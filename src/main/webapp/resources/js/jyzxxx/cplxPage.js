		...
		function getValueFys(){
			...
			$.ajax({
	            ...
	            type: "POST",
	            url: 'getCplxlist1',
	            data: {start: start, limit: size,size: count,cplx: cplx,cpmc: cpmc,jyyj: jyyj},
	            async: false,
...
//		页面刚进来时执行的方法，动态分别加载表格
		window.onload = function(){
			...
			$.ajax({
	            ...
	            type: "POST",
	            url: 'getCplxlist1',
	            data: {start: 0, limit: size,size: count,cplx: cplx,jyyj: jyyj,jyxm: jyxm},
	            async: false,
	            error: function (request) {
	                alert("链接失败,请联系管理员。");
	            },
		    ...
		    var str = "<table cellpadding='0' cellspacing='0' border='0' class='display table table-bordered'>" 
		   	...
		   	for(var i = 0; i < data.length; i++){
		   			str=str+"<td class='center hidden-phone' style='color:red;'>"+(i+1)+"</td>"
			   			   +"<td class='center hidden-phone'>"+isNull(data[i].ZDMC)+"</td>"
			   			   +"<td class='center hidden-phone'><a href='javascript:;' onclick='toView1(\""+data[i].ZDMC+"\")' class='paginate_disabled_previous'>查询产品</a></td>";;
			   		str=str+"</tr>";
			}
		   	str=str+"</tbody></table>";
