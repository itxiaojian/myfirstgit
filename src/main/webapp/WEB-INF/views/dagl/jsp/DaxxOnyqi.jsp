	...
	<div class="panel" style="margin-bottom: 1px;">
		<div class="panel-body">
			...
			<form class="form-horizontal tasi-form" name="myForm" id="myForm" method="post"  style="overflow:scroll;overflow:hidden">
				...
				<input type="hidden" name="id" id="id" value="${daxx.id }">
					...
					<div class="form-group" style="padding-bottom: 2px;margin-bottom: 0px; padding-top: 2px;width: 101%;">
						<label class="col-sm-2 col-sm-2 control-label"><span style="color:red">*</span>保管期限：</label>
						<div class="col-sm-10">
							<select id="selectedRoleId" class="form-control" name="bgqx" >
                            <c:forEach items="${bgqx}" var="bgqx">
                                <c:choose>
                                         <c:when test="${bgqx.zdz == daxx.bgqx}">
                                               <option selected = "selected" value="${bgqx.zdz}">${bgqx.zdmc}</option>
                                         </c:when>
                                         <c:otherwise>
                                               <option value="${bgqx.zdz}">${bgqx.zdmc}</option>
                                         </c:otherwise>
                               </c:choose>
                           </c:forEach> 
                           </select>
