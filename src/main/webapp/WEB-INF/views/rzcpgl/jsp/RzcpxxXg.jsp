...
$(function(){
	...
	$("#myForm").validate({
		submitHandler: function() {
          ...
          msg = "确定要提交？";
          if (confirm(msg)) {
              var url = "<%=path%>/rzcpgl/YRzCpxx/update";
              $.ajax({
                  cache: true,
                  type: "POST",
                  url: url,
...
//下面是一些常用的验证规则扩展
 ...
 <%--  function update(type){
	...
	if(type==0){
		url="<%=path%>/rzcpgl/YRzCpxx/update";
		msg="确定要提交客户信息吗？";
...
<body>
	<div class="wrapper">
		<div class="panel">
			<header class="panel-heading"> 产品认证信息 </header>
			<form class="form-horizontal tasi-form" name="myForm" id="myForm" method="post">
			<c:forEach var="rzCpxx" items="${rzCpxx}" varStatus="obj">
			<input type="hidden" name="id" id="id" value="${rzCpxx.id }">
			<div class="panel-body">
				<label class="col-sm-2 col-sm-2 control-label">产品编号：</label>
					<div class="col-sm-10">
						<input class="form-control" style="width:110%;" type="text" value="${rzCpxx.cpbh }"  name="cpbh" required data-msg-required="产品编号不可空" minlength="1" data-msg-minlength="产品编号不可空">
					</div>
					
					<label class="col-sm-2 col-sm-2 control-label">产品名称：</label>
				<div class="col-sm-10">
					<input class="form-control" type="text" style="width:110%;" value="${rzCpxx.cpmc }" name="cpmc" required data-msg-required="产品名称不可空" minlength="1" data-msg-minlength="产品名称不可空">
				</div>
				...
				<div class="col-sm-10">
					<select class="form-control input-lg m-bot15" name="rzlx" style="font-size:14px; height:25px;width:110%;">
						<option selected="selected">${rzCpxx.rzlx }</option>
				<c:forEach var="rzlx" items="${rzlx}" varStatus="obj">
						<option value="${rzCpxx.rzlx }">${rzlx.zdmc }</option>
				</c:forEach>
					</select>
				</div>
			</div>
			...
			<div class="panel-body">
				...
				<div class="col-sm-10">
					<input class="form-control" type="text" value="${rzCpxx.cpxh }" name="cpxh" style="width:110%;">
				</div>
				
				<label class="col-sm-2 col-sm-2 control-label">认证日期：</label>
				<div class="col-sm-10">
					<input class="form-control" type="text" value="${rzCpxx.rzrq }" style="width:110%;" name="rzrq" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})">
				</div>
				
				<label class="col-sm-2 col-sm-2 control-label">有效期：</label>
				<div class="col-sm-10">
					<input class="form-control" type="text" value="${rzCpxx.yxq }" style="width:110%;" name="yxq" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})">
				</div>
			</div>
			...
			<div class="panel-body">
				...
				<!--拉取当前时间到文本框  -->
						...
						%>
						<input type="text" value="<%=strdate%>" name="xgsj" class="form-control" style="width:110%;"
							onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})">
				</div>
				
				<label class="col-sm-2 col-sm-2 control-label">产品参数：</label>
				<div class="col-sm-10">
					<input class="form-control" type="text" value="${rzCpxx.cpcs }" name="cpcs" style="width:110%;">
				</div>
				
				<label class="col-sm-2 col-sm-2 control-label">生产工厂：</label>
				<div class="col-sm-10">
					<input class="form-control" type="text" value="${rzCpxx.scgc }" name="scgc" style="width:110%;">
				</div>
			</div>
			...
			<div class="panel-body">
				...
				<div class="col-sm-10">
					<input class="form-control" type="text" value="${rzCpxx.bgbh_id }" name="bgbh_id" style="width:110%;">
				</div>
				
				<label class="col-sm-2 col-sm-2 control-label">修改人：</label>
				<div class="col-sm-10">
					<input class="form-control" type="text" value="${rzCpxx.xgr }" name="xgr" style="width:110%;">
				</div>
			</div>
			...
			<div class="panel-body">
				...
				<div class="col-sm-10">
					<textarea  class="form-control" rows="3" cols="" name="bz" id="bz" truetype="textarea" style="width:460%;">${rzCpxx.bz }</textarea>
				</div>
			</div>
