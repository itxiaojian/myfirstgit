...
// 		alert(ysfy);
        var ysje = document.getElementById("xyje").value;
//         alert(ysje);
	    if( ysfy > parseInt(ysje)){
	   	 alert('本次实收金额输入有误，请检查！');
	   	 return false;
	    }
		var url = "<%=path%>/cwgl/YcwJsfwxysf/sFei";
		$.ajax({
			cache : true,
			type : "POST",
			url : url,
		...
		<form class="form-horizontal tasi-form" name="myForm" id="myForm" method="post" >
			...
			<div class="form-group" style="padding-bottom: 2px;padding-top: 0px; margin-bottom: 0px; width: 101%;">
						...
						<c:if test="${xysf.pjfl == null}">
						 ...
						 <c:if test="${xysf.pjfl != null}">
						 <input class="form-control" type="text" readonly="true" id="pjfl" name="pjfl" value="${xysf.pjfl}">
						 </c:if>
						</div>
						...
						<c:if test="${xysf.srfl == null}">
						 ...
						 <c:if test="${xysf.srfl != null}">
						 <input class="form-control" type="text" readonly="true" id="srfl" name="srfl" value="${xysf.srfl}">
						 </c:if>
						</div>
