...
function save() {
	msg="确定提交修改？";
	if (confirm(msg)) {
		var url = "<%=path%>/cwgl/YcwJsfwxysf/jlUpdate";
		$.ajax({
			cache : true,
			type : "POST",
			url : url,
...
<div class="wrapper" >
	<div class="panel" style="margin-bottom: 1px;">
		...
		<form class="form-horizontal tasi-form" name="myForm" id="myForm" method="post" >
			...
			<div class="form-group" style="padding-bottom: 3px;margin-bottom: 0px;margin-top: -6px; width: 101%;">
				<label class="col-sm-2 col-sm-2 control-label">协议金额（元）：</label>
				<div class="col-sm-10">
				<c:if test="${xysf.xyje > 0 }">
					<input class="form-control" type="text" id="xyje" name="xyje" disabled="true" value="${xysf.xyje}">
				</c:if>
				<c:if test="${xysf.xyje < 0 }">
					<input class="form-control" type="text" id="xyje" name="xyje" disabled="true" value="${0-xysf.xyje}">
				</c:if>
				</div>
				<label class="col-sm-2 col-sm-2 control-label">已收金额（元）：</label>
				<div class="col-sm-10">
					<input class="form-control" type="text" id="ysfje" name="ysfje" disabled="true" value="${xysf.ysfje}">
				</div>
				<label class="col-sm-2 col-sm-2 control-label">剩缴金额（元）：</label>
				<div class="col-sm-10">
					<input class="form-control" type="text" id="ysje" name="ysje" disabled="true" value="${xysf.ysje}">
				</div>
			</div>			
			<div class="form-group" style="padding-bottom: 2px;padding-top: 0px; margin-bottom: 0px; width: 101%;">
						...
						<div class="col-sm-10">
<%-- 							<input class="form-control" type="text" id="srfl" name="srfl" disabled="true" value="${xysf.pjfl}"> --%>
                            <c:if test="${xysf.srfl != null}">
							<select id="srfl" class="form-control" name="srfl">
                            <c:forEach items="${srfl}" var="srfl">
                                <c:choose>
                                         <c:when test="${srfl.zdz == xysf.srfl}">
			...
			<div class="form-group" style="padding-bottom: 2px;padding-top: 0px; margin-bottom: 0px; width: 101%;">
						...
						<div class="col-sm-13">
							...
							truetype="textarea" style="width: 99%;height: 33px;">${xysf.bz }</textarea>
						</div>
			</div>
			
			</c:forEach>
			<input style="border: #000" name="num" type="hidden" value="${num}">
			<header class="panel-heading" style="padding-left: 58px;font-size: 11px; font-weight: 0;">缴费记录：</header>
			...
			style="border:0px;table-layout: fixed;width:82%" >
					<thead style="height: 30px;">
						<tr class="">
							...
							<td align="center"><b>已收金额（元）</b></td>
							<td align="center"><b>剩缴金额（元）</b></td>
							<td align="center"><b>本次实收（元）</b></td>
							<td align="center"><b>票据号码</b></td>
							<td align="center"><b>缴费日期</b></td>
							<td align="center"><b>收费人</b></td>
							<td align="center"><b>本次收费备注</b></td>
						</tr>
					</thead>
					<tbody>
						...
						<tr class="" style="height:30px;">
							...
							<td align="center" title="${xysfjl.khmc}">${xysfjl.khmc}</td>
							<td align="center" title="${xysfjl.ssks_id}">${xysfjl.ssks_id}</td>
							<td align="center">${xysfjl.ysfje}</td>
							<td align="center">${xysfjl.ysje}</td>
							<td align="center">${xysfjl.bcss}</td>
							<td align="center"><input style="border: #000" name="id<%=s%>" type="hidden" value="${xysfjl.id}"><input style="border: #000" name="pjhm<%=s%>" value="${xysfjl.pjhm}"></td>
							<td align="center">${xysfjl.sfrq}</td>
							<td align="center">${xysfjl.sfr}</td>
							<td align="center" title="${xysfjl.bz}" style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis;">${xysfjl.bz }</td>
						</tr>
						</c:forEach>
					</tbody>
				</table>
