...
<%
	...
	if("1,3".equals(request.getAttribute("ymids")) || "1,3,4".equals(request.getAttribute("ymids")) || "1,3,5".equals(request.getAttribute("ymids"))){
		//封面
			DataRegion data1 = worddoc.openDataRegion("PO_p2");
...
// 			data3.setValue("[word]"+request.getSession().getServletContext().getRealPath("resources/doc")+"/"+bgbh+"fy"+".doc"+"[/word]");
// 		}
	}else if("1,3,4,5".equals(request.getAttribute("ymids"))){
		//封面
			DataRegion data1 = worddoc.openDataRegion("PO_p2");
...
// 			data3.setValue("[word]"+request.getSession().getServletContext().getRealPath("resources/doc")+"/"+bgbh+"fy"+".doc"+"[/word]");
// 		}
	}else if("2,3".equals(request.getAttribute("ymids"))){
		//首页
...
// 	poCtrl1.addCustomToolButton("另存为PDF文件", "SaveAsPDF()", 1);
	...
	if("0".equals( request.getAttribute("fl")) || "4".equals( request.getAttribute("fl")) || "6".equals( request.getAttribute("fl"))){
		poCtrl1.webOpen("/zjyw/resources/doc/ybbgfsfy.doc", OpenModeType.docReadOnly, ""); //docNormalEdit
	}else if("1".equals( request.getAttribute("fl")) ){
		poCtrl1.webOpen("/zjyw/resources/doc/gpbgfsfy.doc", OpenModeType.docReadOnly, "");
	}else if("2".equals( request.getAttribute("fl")) ||"5".equals( request.getAttribute("fl")) ){
		poCtrl1.webOpen("/zjyw/resources/doc/gjbgfsfy.doc", OpenModeType.docReadOnly, "");
	}
