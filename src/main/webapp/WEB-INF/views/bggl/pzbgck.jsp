...
<%
	...
	poCtrl1.setCaption("报告批准");
	WordDocument worddoc = new WordDocument();
	
	if("0".equals( request.getAttribute("bztsbg"))){
		DataRegion data2 = worddoc.openDataRegion("PO_p2");
		data2.setValue("[word]"+request.getSession().getServletContext().getRealPath("resources/doc")+"/"+bgbh+"sypz"+".doc"+"[/word]");
	}
...
<html>
	...
	<script type="text/javascript">
			function AfterDocumentOpened(){
			 	document.getElementById("PageOfficeCtrl1").WebSaveAsPDF();   //word文件转pdf 
			}
			function Zuida(){
	        ...
	        var tname =  $("#tname").val();
	        function CS(){
var openUrl = "<%=path%>/jygl/YjyBgxx/CS?bgbh="+bgbh+"&userCode="+userCode+"&taskId="+taskId+"&businessKey="+creditId+"&lineVar="+lineVar+"&tname="+tname+"&cs=2";
			...
			function agree() {
				...
				if(returnFlag) {
                    ...
                    if (confirm(msg)) {
                    	var bgbh = "${bgbh}";
            			var url = "<%=path%>/jygl/YjyBgxx/deletefylc";
            		    $.ajax({
            		        cache: true,
            		        type: "POST",
            		        url: url,
