			...
			<div style="text-align:center;margin-bottom:-2px"></div>
                ...
                <c:forEach var="sbxx" items="${sbxx}" varStatus="obj">
					...
					<div class="form-group" style="padding-bottom: 2px;margin-bottom: 0px;margin-top: 4px; width: 101%;">
						<label class="col-sm-2 col-sm-2 control-label"><span style="color:red">*</span>报废原因：</label>
						<div class="col-sm-10">
							<input class="form-control" type="text" id="bfyy" name="bfyy" ">
						</div>
						<label class="col-sm-2 col-sm-2 control-label"><span style="color:red">*</span>报废日期：</label>
						<div class="col-sm-10">
							<input name="bfrq" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="form-control Wdate" />
						</div>
				   </div>
...
function save() {
	msg="确定要提交？";
	if (confirm(msg)) {
		var url = "<%=path%>/sbgl/YSbXx/onBfei";
		$.ajax({
			cache : true,
			type : "POST",
			url : url,
