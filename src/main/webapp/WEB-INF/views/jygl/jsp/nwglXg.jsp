            ...
            success: function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    str = str + "<tr id='id" + i + "' name='id" + i + "'><td style='width:80px'>" + isNull(data[i].YPMC) + "</td><td style='width:60px'>" + isNull(data[i].WTDW) + "</td>"
                            + "<td style='width:50px'>" + isNull(data[i].GGXH) + "</td><td style='width:50px'>" + isNull(data[i].JYYJ) + "</td>"
                            + "<td><input type ='button' onClick='setYpxx(\""+isNull(data[i].YPMC)+"\",\""+data[i].YPBH+"\",\""+isNull(data[i].GGXH)+"\",\""+isNull(data[i].JYYJ)+"\");' value='选择'></td>";
	...
	<!-- <header class="panel-heading" style="padding-left: 650px;"> 科室收入</header> -->
		...
		<input type="hidden" name="nwbh" id="nwbh" value="${nwxx.id }">
				...
				<div class="col-sm-10">
					<select class="form-control input-lg m-bot15" id="cjbm" name="cjbm" onchange="addOptions(this.value);">
						...
						<c:forEach var="data" items="${cjbm }" varStatus="obj">
							<option value="${data.BMBH }" <c:if test="${nwxx.cjbm==data.BMBH }">selected = "selected"</c:if> >${data.BMMC }</option>
						</c:forEach>
					</select>
				</div>
			...
			<div class="form-group" style="padding-bottom: 10px;padding-left: 0px;">
				...
				<div class="col-sm-10">
					<input class="form-control Wdate" type="text" id="yqwcrq" name="yqwcrq" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="${nwxx.yqwcrq }">
				</div>
			</div>
			...
			<div class="form-group" id="xsDiv" style="margin-top:50px">
				<table style="width: 80%; height: 76px;margin-left: 95px;" id="xsTab">
					...
					<tr id="mybody${obj.count }">
						...
						data-toggle="modal" href="#myModal" onclick="getNumber(${obj.count });">选择委托样品</a></label>
	                 	</td>
						<td><input type="hidden" name="mxid${obj.count }" id="mxid${obj.count }" value="${data.id }">
						...
						<td><textarea style="height: 37px;" class="form-control" type="text" id="jyxm${obj.count }" name="jyxm${obj.count }" >${data.jyxm }</textarea></td>
						<td><textarea style="height: 37px;" class="form-control" type="text" id="je${obj.count }" name="je${obj.count }" >${data.jyfy }</textarea></td>
						<td style="width:10%">
							<select style="height: 37px;" class="form-control input-lg m-bot15" id="sfty${obj.count }" name="sfty${obj.count }">
								<option value="0" <c:if test="${data.sfty=='0' }">selected = "selected"</c:if> >是</option>
								<option value="1" <c:if test="${data.sfty=='1' }">selected = "selected"</c:if> >否</option>
							</select>
						</td>
						<td><textarea style="height: 37px;" class="form-control" type="text" id="bcqx${obj.count }" name="bcqx${obj.count }" value="/"></textarea></td>
			...
			<div style="text-align:center;margin-bottom:10px">
                ...
                &nbsp;&nbsp;&nbsp;
				<input class="btn btn-success" value="提交" type="button" onclick="save();" style="height: 36px;">
				&nbsp;&nbsp;&nbsp;
