	...
	<form class="tasi-form" name="myForm" id="myForm" method="post"  style="overflow:scroll;overflow-x:hidden">
					...
					<input type="hidden" name="id" value="${id }" >
						...
						<div class="col-sm-10 srk">
							<select class="form-control input-lg m-bot15" name="jllb1" style="padding-left: 2px;" >
								<c:forEach var="jl1" items="${jl1}" varStatus="obj" >
									<c:choose>
                              			<c:when test="${jl1.zdz == jllb1}">
                                    		<option selected = "selected" value="${jl1.zdz}">${jl1.zdmc}</option>
                              			</c:when>
                              			<c:otherwise>
                                    		<option value="${jl1.zdz}">${jl1.zdmc}</option>
                              			</c:otherwise>
                          			</c:choose>
								</c:forEach>
					        </select>
						...
						<div class="col-sm-10 srk">
							<select class="form-control input-lg m-bot15" name="jllb2" style="padding-left: 2px;" >
								<c:forEach var="jl2" items="${jl2}" varStatus="obj" >
									<c:choose>
                             			<c:when test="${jl2.zdz == jllb2}">
                                   		<option selected = "selected" value="${jl2.zdz}">${jl2.zdmc}</option>
                             			</c:when>
                             			<c:otherwise>
                                   		<option value="${jl2.zdz}">${jl2.zdmc}</option>
                             			</c:otherwise>
                         			</c:choose>
								</c:forEach>
					        </select>
					...
					<div class="form-group div" style="height:40px;">
						...
						<div class="col-sm-13 srk">
							<textarea class="form-control ckeditor textarea wbk" rows="6"  name="cgjlyy" 
							 style="width: 99.6%;height: 35px;resize:none;">${cgjlyy }</textarea>
						</div>
					</div>
					<div class="form-group div">
						...
						<div class="col-sm-13 srk">
							...
							truetype="textarea" style="width: 99.6%;height: 35px;resize:none;">${bz }</textarea>
						</div>
					</div>
			</form>
	</div>
	        <div align="center">
                <br>
                <input class="btn btn-success" value="修改" type="button"  onclick="add();">
...
function add() {
	msg="确定要提交？";
	if (confirm(msg)) {
		var url = "<%=path%>/jygl/YjyClyy/update";
		$.ajax({
			cache : true,
			type : "POST",
			url : url,
