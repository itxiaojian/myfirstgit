...
<!-- 								<input class="btn btn2 btn-success" type="button" style="margin-left: 3%;" onclick="getExcelValue();" value="导出Excel"> -->
				...
				<div class="panel-body" style="margin-top: 200px;">
					<div class="adv-table" id="Div1">
						<div id="Div6" style="overflow-x:auto;height:342px;">
						...
						<div id="Div4">
								<div id="table_length" class="dataTables_length" style="float:left;margin-top: 6px;padding-bottom: 0px;padding-top: 6px;">
									<label>
										每页显示
								...
								<div id="Div5" class="Div5" style="margin-top: 6px;">
...
<script type="text/javascript">
    	...
    	<%
    	// 这段可以用EL和JSTL等方法代替
     	List<Map<String,Object>> syks=(List)request.getAttribute("jyksList");
     	for (Map<String,Object> currentValue : syks) {%>
    	// 这段服务器端得部分可以用EL和JSTL代替
    	idList.push("<%=currentValue.get("id")%>");
    	mcList.push("<%=currentValue.get("bmmc")%>");
    	<% } %>
    	...
    	<%
    	// 这段可以用EL和JSTL等方法代替
     	List<Map<String,Object>> jyzq=(List)request.getAttribute("jyzq");
     	for (Map<String,Object> currentValue : jyzq) {%>
    	// 这段服务器端得部分可以用EL和JSTL代替
    	jyzqidList.push("<%=currentValue.get("zdz")%>");
    	jyzqmcList.push("<%=currentValue.get("zdmc")%>");
    	<% } %>
    	...
    	<%
    	// 这段可以用EL和JSTL等方法代替
     	List<Map<String,Object>> jddw=(List)request.getAttribute("jddw");
     	for (Map<String,Object> currentValue : jddw) {%>
    	// 这段服务器端得部分可以用EL和JSTL代替
    	jddwidList.push("<%=currentValue.get("zdz")%>");
    	jddwmcList.push("<%=currentValue.get("zdmc")%>");
    	<% } %>
    	...
    	<%
    	// 这段可以用EL和JSTL等方法代替
     	List<Map<String,Object>> sbzt=(List)request.getAttribute("sbzt");
     	for (Map<String,Object> currentValue : sbzt) {%>
    	// 这段服务器端得部分可以用EL和JSTL代替
    	sbztidList.push("<%=currentValue.get("zdz")%>");
    	sbztmcList.push("<%=currentValue.get("zdmc")%>");
    	<% } %>
    	...
    	<%
    	// 这段可以用EL和JSTL等方法代替
     	List<Map<String,Object>> syzt=(List)request.getAttribute("syzt");
     	for (Map<String,Object> currentValue : syzt) {%>
    	// 这段服务器端得部分可以用EL和JSTL代替
    	syztidList.push("<%=currentValue.get("zdz")%>");
    	syztmcList.push("<%=currentValue.get("zdmc")%>");
    	<% } %>
    </script>
<script type="text/javascript" src="<%=path%>/resources/js/tjgl/sbcx.js"></script>
