...
<head>
	...
	<script type="text/javascript" src="<%=path%>/resources/js/weixin/staticData.js"></script>
 	<script type="text/javascript" src="<%=path%>/resources/js/weixin/user/wxGroup.js"></script>
...
<style>
	.x-grid-record-red table{ background: #FFC0CB;} 
	.hide{display:none;}
...
<body style="background-color: #ffffff;">
	<div style="display: none;">
		<ul class="tab-menu tab" id="tabMenuID_">
			<li class="tab-selected" title="用户分组管理"><a
				href="<%=path%>/wxauth/group/wxGroup" target="content"
				onfocus="this.blur()"><span>用户分组管理</span></a></li>
		</ul>
	</div>
	<form method="post" id="searchForm" action="<%=path%>/sys/SysUser/list">
		<input type="hidden" id="startId" name="start" value="0" /> <input
			type="hidden" id="limitId" name="limit" value="10" />
	</form>
