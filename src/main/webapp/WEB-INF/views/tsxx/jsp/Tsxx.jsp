...
function openwin() {  
   ...
   // location.href="Yh?yhbh="+yhbh;
    window.open("<%=path%>/tsxx/YtsXx/Yh?yhbh=="+yhbh, "用户信息", "height=750, width=800, toolbar =no, menubar=no, scrollbars=yes, resizable=no, location=no, status=no") ;
			...
			<form action="" class="form-horizontal tasi-form bl-form bl-formhor" name="myForm" id="myForm" method="post">
			 <div class="panel-body">
					...
					<div class="col-sm-10">
						...
						<input class="form-control" name="btsr" id="btsr" placeholder="--请选择--"   required data-msg-required="被投诉人必填" minlength="1" data-msg-minlength="被投诉人必填"
				...
				<div class="panel-body">
					...
					<div class="col-sm-10">
						<textarea class="form-control "  truetype="textarea" placeholder="最少输入10个字" value="" name="tsnr" style="width: 400%;height: 80px;" required data-msg-required="最少输入10个字符" minlength="10" data-msg-minlength="最少输入10个字符"></textarea>
					</div>
				</div>

				<div class="panel-body">
					<label class="hidden">处理状态</label>
					<div class="col-sm-10">
						<select class="form-control input-lg m-bot15" name="clzt"  style="display:none"
							style="font-size: 14px; height: 40px;">
							<c:forEach var="clzt" items="${clzt}" varStatus="obj">
								<option value="${clzt.zdz }"
									<c:if test="${clzt.zdz=='1' }">selected="selected"
									</c:if>
								>${clzt.zdmc}
								</option>
							</c:forEach>
				...
				<div class="panel-body">
					...
					<label class="hidden" style="left:200px;">投诉日期：</label>
					<div class="col-sm-10" style="margin-left:26px;">
						<!--拉取当前时间到文本框  -->
						<%
							Date date = new Date();
						...
						%>
						<input type="hidden" value="<%=strdate%>" name="tsrq"
							class="form-control"
							onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" style="width:160%;height:30px; margin-left:175px;left:-40px;">
					</div>
				</div>
				
				<div class="panel-body">
					<label class="hidden">投诉人</label>
					<div class="col-sm-10">
						<input class="form-control" type="hidden" value="" name="tsr" style="width:100px;height:30px;">
					</div>
				</div>
