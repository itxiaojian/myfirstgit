var ENTITY_URL_LIST = "/wxpt/wsh/ShWxcjpz/pager";
var ENTITY_URL_SAVE = "/wxpt/wsh/ShWxcjpz/save";
var ENTITY_URL_UPDATE = "/wxpt/wsh/ShWxcjpz/update";
var ENTITY_URL_DELETE = "/wxpt/wsh/ShWxcjpz/delete";
var PAGESIZE=50;
/*
 * 微信抽奖配置 
 * @author liujiansen
    ...
    onModifyClick: function() {
    	var records = this.getSelectionModel().getSelections();
   		if(records.length > 0) {
   			if(records.length == 1){
   				vrecord = records[0];
				if(vrecord.get('jssj')!='2999-12-31'){
	    			Ext.Msg.alert('系统提示','只能修改结束日期为2999-12-31的数据！');
	    			return false;
	    		}
   				if(!this.shWxcjpzUpdateWindow)
					this.shWxcjpzUpdateWindow = new ShWxcjpzUpdateWindow();
   				
   		    	var win = this.shWxcjpzUpdateWindow;
    ...
    onDeleteClick: function(){
		...
		if(records.length > 0) {
			if(records.length==1){
				...
				if(vrecord.get('jssj')!='2999-12-31'){
	    			Ext.Msg.alert('系统提示','只能删除结束日期为2999-12-31的数据！');
	    			return false;
	    		}
