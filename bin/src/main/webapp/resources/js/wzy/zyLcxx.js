var USER_GRID_STORE_URL = 'getLcxxList';
var PAGESIZE=20;
    ...
    constructor: function(height, width){
    	this.store = new Ext.data.Store({          //Grid Store
            ...
            reader: new Ext.data.JsonReader({totalProperty: 'total', root:'rows'},[
            		{name:'id'},{name:'lczl'},{name:'lcmc'},{name:'kssj'},{name:'jssj'},{name:'fqr'},{name:'fqrxm'}
            		,{name:'bmmc'},{name:'lcnr'},{name:'bmbh'},{name:'lczt'}
            ])
        });
...
//            	{header:'新闻标题',dataIndex:'xwbt',width:200,sortable: true},
            	{header:'流程名称',dataIndex:'lcmc',width:200,sortable: true, 
                    renderer:function(value, cellmeta, record){
                    	...
                    	return '<a target="_blank" href="zyLcxxDetail?id=' + id + '">' + value + '</a>';
                    }
            	},
            	{header:'发起人姓名',dataIndex:'fqrxm',width:200,sortable: true},
            	{header:'部门名称',dataIndex:'bmmc',width:200,sortable: true},
            	{header:'开始时间',dataIndex:'kssj',width:200,sortable: true},
            	{header:'流程种类',dataIndex:'lczl',width:260,sortable: true, 
            		renderer:function(value){
	                    if(value == '1') {
	                        return "<span style='color:blue;'>出差申请</span>";
	                    }else if(value == '2') {
	                        return "<span style='color:red;'>请假申请</span>";
	                    }else if(value == '3') {
	                        return "<span style='color:fuchsia;'>调课申请</span>";
	                    }else if(value == '4') {
	                        return "<span style='color:lime ;'>报修</span>";
	                    }else{
	                        return value;
	                    }
            		}
    ...
    addByEditor: function() {//用网页编辑器编辑新闻
    	var url = "zyLcxxListPage";   	
    	html = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="' + url + '"></iframe>';    		
    	ACT_DEAL_WINDOW = new ActDealWindow();
    	ACT_DEAL_WINDOW.setTitle("发布流程信息");
