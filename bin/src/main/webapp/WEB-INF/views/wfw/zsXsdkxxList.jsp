...
<ul class="tab-menu tab" id="tabMenuID_">
	<li class="tab-selected" title="学生贷款信息">
	<a href="#" target="content" onfocus="this.blur()"><span>学生贷款信息</span></a>
	</li>
</ul>
</div>
<div class="bg1">
	<div class="style1">
		...
		<a class="font1" href="<%=path%>/wfw/ZsXsdkxx/toXsdkxx?openId=${openId }">学号&姓名</a>
		&nbsp;&nbsp;&nbsp;
			<!-- <span class="font1" >学号</span> -->
		</div>
		<form action="<%=path%>/wfw/ZsXsdkxx/toXsdkxx" id="myForm" class="wwx_f_a"  method="post">
...
<div>
	...
	<div class="style4" style="text-align: center;">
				<c:choose>
					<c:when test="${pages > 1}">
						<a href="<%=path%>/wfw/ZsXsdkxx/toXsdkxx?pages=${pages - 1}&openId=${openId}">上一页</a>
					</c:when>
					<c:otherwise>
						上一页
					</c:otherwise>
				...
				<c:choose>
					<c:when test="${pages < count}">
						<a href="<%=path%>/wfw/ZsXsdkxx/toXsdkxx?pages=${pages + 1}&openId=${openId}">下一页</a>
					</c:when>
					<c:otherwise>
					下一页
				</c:otherwise>
