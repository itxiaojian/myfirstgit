   ...
   <script src="<%=path%>/resources/mobiscroll/js/mobiscroll.datetime-2.5.1-zh.js" type="text/javascript"></script>
	...
	function toView(id,time,openId){
		...
		if(a>0){
			...
			location.href ='<%=path%>/wsh/ShWbm/toWbmDetail?id='+id+'&openId='+openId;
		}
	}
	function query(){
		$('#Form1').submit();
				...
				<input type="hidden" name="openId" id="openId" value="${openId }"> 
					<table border="0" align="center" cellpadding="0" style="margin-top:80px;margin-bottom: 10px;width:100%; "
						...
						<tr class="bgcolor03">
						    ...
						    <td width="20%" align="center" style="font-weight:bold;height:1%;">名称</td>
						    <td width="30%" align="center" style="font-weight:bold;height:1%;">截止时间</td>
						    <td width="20%" align="center" style="font-weight:bold;height:1%;">报名结果</td>
						...
						<c:if test="${obj.count%2 == '0'}">
							<tr class="bgcolor01">
							    ...
							    <td align="center">${data.bmbt}</td>
							    <td align="center">${data.bmjzsj}</td>
							    <td><a href="<%=path%>/wsh/ShWbm/toBmrsSj?id=${data.id }&openId=${openId}" style="color: blue;">${data.bmrs }人报名 »</a></td>
							    <td align="center"><a href="#" onclick="toView('${data.id }','${data.bmjzsj }','${openId }');" style="color: blue;">点击报名</a></td>
						  </tr>
					 </c:if>
					 <!-- 判断奇数行 -->
					<c:if test="${obj.count%2 != '0'}">
			 ...
			 <c:if test="${!empty list}">
			 		<tr>
			 			<td align="center" colspan="5">
			 				<c:choose>
								<c:when test="${pages > 1}">
									<a href="<%=path%>/wsh/ShWbm/toWbmSjList?pages=${pages - 1}&openId=${openId}">上一页</a>
								</c:when>
								<c:otherwise>
									上一页
								</c:otherwise>
							...
							<c:choose>
								<c:when test="${pages < count}">
									<a href="<%=path%>/wsh/ShWbm/toWbmSjList?pages=${pages + 1}&openId=${openId}">下一页</a>
								</c:when>
								<c:otherwise>
								下一页
							</c:otherwise>
