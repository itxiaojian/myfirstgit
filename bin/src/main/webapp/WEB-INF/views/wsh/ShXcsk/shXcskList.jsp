...
<div class="bg1">
	<div class="style1">
		<div class="wwx_f_l" style="position: relative;top:6px;">
			<a id ="lianjie1" class="font1" href="/wxpt/wsh/ShXcsk/toShxcskList">车牌号</a>
		</div>
		<form action="/wxpt/wsh/ShXcsk/toShxcskList" id="myForm" class="wwx_f_l" style="width: 55%;margin-right: 10px;" method="post">
...
<div>
	...
	<c:forEach var="data" items="${list}" varStatus="obj">
		<div class="maring1">
				<div class="wwx_f_l" style="width:60%" id='divs'>
					<p class="title">
						<a id = 'lianjie' href="/wxpt/wsh/ShXcsk/toShxcskDetail?cph=${data.cph }">
	...
	<div class="style4" style="text-align: center;">
				<c:choose>
					<c:when test="${pages > 1}">
						<a href="/wxpt/wsh/ShXcsk/toShxcskList?pages=${pages - 1}">上一页</a>
					</c:when>
					<c:otherwise>
						上一页
					</c:otherwise>
				...
				<c:choose>
					<c:when test="${pages < count}">
						<a href="/wxpt/wsh/ShXcsk/toShxcskList?pages=${pages + 1}">下一页</a>
					</c:when>
					<c:otherwise>
					下一页
				</c:otherwise>
