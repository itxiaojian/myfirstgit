...
<script type="text/javascript">
	...
	function Sc(id){
		var ret = window.confirm("确定要删除这条信息吗？");
		if (!ret) {
			return false;
		}
		var url = '<%=path%>/wsh/ShWsq/delete';
		$.ajax({
			url: url,
			type : 'post',
			dataType : 'json',
			...
			success: function(data){
				if(data == '1'){
					alert("删除成功");
					location.href ="<%=path%>/wsh/ShWsq/toWsqList";
				}else{
					alert("删除失败");
				}
			},
	...
	function toView(id){
<%-- 		location.href ='<%=path%>/wsh/ShWsq/toWsqView?id='+id --%>
		window.open('<%=path%>/wsh/ShWsq/toWsqView?id='+id, "大屏幕");
	}
</script>
...
<script type="text/javascript" src="<%=path%>/system/login/js/jquery.js"></script>
	...
	<body>
		...
		<ul class="tab-menu tab" id="tabMenuID_">
			<li class="tab-selected" title="微上墙">
			<a href="<%=path%>/wsh/ShWsq/toWsqList" target="content" onfocus="this.blur()"><span>微上墙</span></a>
			</li>
		</ul>
		</div>
		<div class="container">
...
<a href="<%=path%>/wsh/ShWsq/toWsqAdd" class="btn btn-success add-apply-btn">+创建上墙</a>

<div>
		<table class="table table-hover">
		<thead>
			<tr>
				<th class="span1" style="width: 20%">名称</th>
				<th style="width: 20%">时间</th>
				<th class="span2" style="width: 20%">概览</th>
				<th class="span2" style="width: 25%">操作</th>
				<th class="span3" style="width: 15%">状态</th>
			</tr>
		</thead>
		<tbody>
			<c:forEach var="data" items="${list}" varStatus="obj">
						<tr>
				<td><a href="#" onclick="toView(${data.id});">${data.hdmc }</a></td>
				<td>${data.kssj }~<br/>${data.jzsj }</td>
				<td>${data.cyrs }人参与<br/>${data.fbxxs }条信息</td>
				<td>
					<a href="<%=path%>/wsh/ShWsq/toWsqUpdate?id=${data.id }">编辑</a>
					<a href="#" class="js-del" onclick="Sc(${data.id });">删除</a> 
					<c:if test="${data.res==1 }">
						...
						<a href="<%=path%>/wsh/ShWsq/toWsqYhqd?id=${data.id }">预览</a>
					</c:if>
				</td>
				<td>
					<c:if test="${data.res==0 }"><span style="color: #fff;padding: 2px 5px;background-color: #b2b2b2;">已结束</span></c:if>
					<c:if test="${data.res==1 }"><span style="color: #fff;padding: 2px 5px;background-color: #fd9e0d;">进行中</span></c:if>
					<c:if test="${data.res==2 }"><span style="color: #fff;padding: 2px 5px;background-color: #fd9e0d;">未开始</span></c:if>
				</td>
			</tr>
			</c:forEach>
				</tbody>
...
<div id="footer">
	<c:choose>
					<c:when test="${pages > 1}">
						<a href="<%=path%>/wsh/ShWsq/toWsqList?pages=${pages - 1}">上一页</a>
					</c:when>
					<c:otherwise>
						上一页
					</c:otherwise>
				...
				<c:choose>
					<c:when test="${pages < count}">
						<a href="<%=path%>/wsh/ShWsq/toWsqList?pages=${pages + 1}">下一页</a>
					</c:when>
					<c:otherwise>
					下一页
				</c:otherwise>
