...
<div>
		...
		<tbody>
			<c:forEach var="data" items="${list}" varStatus="obj">
						<tr>
				<td>${obj.count }</td>
				<td><a href="<%=path%>/wsh/WjObject/toWjDetail?id=${data.oid}">${data.title}</a></td>
					<c:if test="${data.state eq '1'}">
						<td>已发布</td>
						<td>${data.createtime}</td>
						<td>
							<a href="#" onclick="cancel_onClick(${data.oid},${data.state});">撤销</a>| 
							<a href="#" onclick="term_onClick(${data.oid},${data.state});">终止评议</a>|
							<a href="#" onclick="Modify_onClick(${data.oid});">修改</a>|
							<a href="#" onclick="Delete_onClick(${data.oid});">删除</a>|
							<a href="#" onclick="QuestionEdit_onClick(${data.oid},${data.state});">编辑表单</a>|
							<a href="#" onclick='preview_onClick2(${data.oid},${data.state})'>预览</a>|
							<a href="#" onclick='showResult_onClick(${data.oid},${data.state});'>查看结果</a>
						</td>
					</c:if>
					<c:if test="${data.state eq '0'}">
						...
						<td>${data.createtime}</td>
						<td>
							<a href="#" onclick="publish_onClick(${data.oid},${data.state});">发布</a>| 
							<a href="#" onclick="term_onClick(${data.oid},${data.state});">终止评议</a>|
							<a href="#" onclick="Modify_onClick(${data.oid});">修改</a>|
							<a href="#" onclick="Delete_onClick(${data.oid});">删除</a>|
							<a href="#" onclick="QuestionEdit_onClick(${data.oid},${data.state});">编辑表单</a>|
