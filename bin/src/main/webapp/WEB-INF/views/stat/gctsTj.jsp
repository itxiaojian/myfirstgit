...
<body>
    ...
    function showECharts(){
        $.ajax({
			  type: 'POST',
			  url: '<%=path%>/stat/echarts/getdatalib',
	  ...
	  $(function() {
		  $(".chzn-select").chosen().change(function(){
				...
				return str;
			     ...
			     $.ajax({
					url :'<%=path%>/stat/echarts/todatalib',
					data : {
						id :test(),
							//$('.chzn-single').text(),
					},
...
<body onload="showECharts()">
	...
	<div id="table"
		...
		<select class="chzn-select" data-placeholder="图书种类"
			...
			<option value="${data.id}">${data.zl}</option>
			</c:forEach>
		</select>
	</div>
		<table class="bordered">
			<thead id="thead">
				<tr>
					...
					<td height="20" style="width: 305px;">图书种类</td>
				...
				<c:forEach var="data" items="${list}" varStatus="status">
					<c:if test="${status.index % 2 == 0}">
						<tr class="even">
							...
							<td><font size="2px">${data.zl}</font></td>
