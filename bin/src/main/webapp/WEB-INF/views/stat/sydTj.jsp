...
<body>
    ...
    function showECharts(){
        $.ajax({
			  type: 'POST',
			  url: '<%=path%>/stat/echarts/tochartsyd',
	  ...
	  $(function() {
		  $(".chzn-select").chosen().change(function(){
				...
				return str;
			     ...
			     $.ajax({
					url :'<%=path%>/stat/echarts/gettablesyd',
					data : {
						id :test(),
							//$('.chzn-single').text(),
					},
...
<body onload="showECharts()">
	...
	<div id="table"
		...
		<div>
		<select class="chzn-select" data-placeholder="生源地"
			style="width:40%;" tabindex="1">
			<option value=""></option>
			<option value="">全部</option>
			<c:forEach var="data" items="${listBm}" varStatus="status">
			<option value="${data.sfbh}">${data.xssf}</option>
			</c:forEach>
		</select>
	</div>
		<table class="bordered">
			<thead id="thead">
				<tr>
					...
					<td height="20" style="width: 305px;">生源地</td>
				...
				<c:forEach var="data" items="${list}" varStatus="status">
					<c:if test="${status.index % 2 == 0}">
						<tr class="even">
							...
							<td><font size="2px">${data.xssf}</font></td>
